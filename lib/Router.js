...
class Router {
    ...
    */
    ...
    constructor(config,ExceptionHandler) {
        if(ExceptionHandler){
            this.newException = ExceptionHandler.newException;
        }else{
            this.newException = this.createVirtualExceptionHandler();
        }
    ...
    pageContainerDidMount() {
        ...
        if(!this.containerDom){
            this.newException("router can't find dom with "+this.pageContainerTagId+" ID maybe your html page is not ready yet");
             return false;
        }else{
    ...
    set loadState(newValue){
        if(newValue>4 && newValue <10){
            ...
            this.newException({invalidValue: newValue},"مقدار ست شده برای وضعیت لودینگ نمیتواند کمتر پنج و بیشتر از ده باشد \n")
        }
    }
    ...
    loadPage(absolutePath , updateHistory) {
        ...
        if(!this.isPageContainerMounted) {
            this.newException("router can`t load route and render it before 'pageContainerDidMount' function called");
            return false;
        }
        ...
        if(normalizedPath != -1) {
            ...
            if(matchedRoute) {
                ...
                if(!(matchedRoute.componentPath != null && matchedRoute.componentPath != undefined)){
                    ...
                    this.newException(new Error(matchedRoute),"the matched Route we find dosent have any component path to load. /n please define component path so we can load and place it")
                }
                ...
                this.newException(new Error(absolutePath),"آدرس وارد شده یافت نشد");
                return(-1);
            }
    ...
    _onRouteLoadComplete(route,componentClass){
        ...
        route.loadge(componentClass,this.containerDom).then((componentClass)=>{
            ...
            this.newException(e,"we have exception in loadge the page. \n");
            //TODO: load 500 page error here
            this._onRouteLoadError(e,reactRoute);
        })
    }
    _onRouteLoadError(e,route){
        this.newException(e,"we had a problem to load component");
...
_normalizePath(path) {
    ...
    createVirtualExceptionHandler(){
        //if user dont set special exception handler for module we just keep small functionallity by cconsole error
        return function newException(exception , message){
            console.error(message,null,exception);
        }
    }
